...
function PostList(props) {
  ...
  const [PostList, setPostList] = useState("로딩중입니다.");
  useEffect(() => {
    setTimeout(() => {
  ...
  if (fakePost.length == 0) {
  }
  return (
    <>
      <h1 className="Head">{PostList}</h1>
      {fakePosts.map((value) => {
        return (
...
export default function Prac1() {
  ...
  const [fakePost, setFakePosts] = useState(fakePosts);
  return (
    <>
      <PostList fakePost={fakePost} />
    </>
  );
}
